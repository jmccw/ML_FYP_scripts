...
class Material {
	...
	public:
		// Constructor
		Material(std::string name_, double electronAffinity_, double bandGap_, double eEffMass_, double lhEffMass_, double hhEffMass_)
			: name(name_), electronAffinity(electronAffinity_), bandGap(bandGap_), eEffMass(eEffMass_), lhEffMass(lhEffMass_), hhEffMass(hhEffMass_) {}
		// Default constructor
		Material() : name(""), electronAffinity(0.0), bandGap(0.0), eEffMass(0.0), lhEffMass(0.0), hhEffMass(0.0) {}
	...
	public:
		...
		Heterostructure(const std::string& materialFileName, const std::string& layerFileName) : potential_(3, vector<double>(number_steps)) {
			...
			while (std::getline(materialFile, line)) {
				...
				double bandGap, electronAffinity, eEffMass, lhEffMass, hhEffMass;
				if (!(iss >> materialName >> bandGap >> electronAffinity >> eEffMass >> lhEffMass >> hhEffMass)) {
					throw std::runtime_error("Error: Invalid material file format");
				}
				// Create Material object and store in the map
				Material material(materialName, bandGap, electronAffinity, eEffMass, lhEffMass, hhEffMass);
		...
		double getPotential(int particle, double x, int x_int) {
			if (particle == 1 || particle == 2){ // if particle is hole
				return this->potential_[particle][x_int] - electron_charge*electric_field*(x-this->getThickness()/2.0); // not as efficient as defining a strict function globally but gives ALOT of freedom (only adds +(number of layers)*3 computations at most  
			}
		...
		void display() { // prints heterostructure details
			cout << "Layers: " << endl;
			int i = 1;
...
void plot_potential(Heterostructure& QW){
    ...
    gnuplot_two_functions ("Potentials", "linespoints", "x [A]", "Effective Potentials",
			x_test, potential1, number_steps, "Valence", x_test, potential2, number_steps, "Conduction");
}
...
void ShiftBandgapQWI(Heterostructure& QW, double shift_amount){ 					// shift amount in nm ?
	...
	//~ for (int i = 0; i < number_steps; i++) {
		//~ y_array_2[i] = eigenVectors[1][0][i];
		//~ y_array_1[i] = eigenVectors[1][5][i];
		//~ x_array_2[i] = i;
	//~ }
	//~ gnuplot_two_functions ("Numerical Eigenfunctions", "linespoints", "x", "Eigenfunction |E_1> (x)",
		//~ x_array_2, y_array_2, number_steps, "Ground", x_array_2, y_array_1, number_steps, "5th excited state");
	...
	//~ for (int i = 0; i < number_steps; i++) {
		//~ y_array_2[i] = eigenVectors[2][0][i];
		//~ y_array_1[i] = eigenVectors[2][5][i];
		//~ x_array_2[i] = i;
	//~ }
	//~ gnuplot_two_functions ("Numerical Eigenfunctions", "linespoints", "x", "Eigenfunction |E_1> (x)",
		//~ x_array_2, y_array_2, number_steps, "Ground", x_array_2, y_array_1, number_steps, "5th excited state");
...
void script(){
	...
	delta_field *= 0.0001; //conversion to V/Angstrom
		
	Heterostructure QW("materials.txt","input.txt");
	QW.display();
	plot_potential(QW);
	if((double)params.targetBandgapShift>0) {
	...
	cout << energies_relative[0][0]<<endl;
	cout << energies_relative[2][0]<<endl;
	cout << abs(energies_relative[2][0]-energies_relative[0][0])<<endl;
	
	double x_array_2[number_steps], y_array_2[number_steps], y_array_1[number_steps];
	for (int i = 0; i < number_steps; i++) {
		y_array_2[i] = eigenVectors[0][0][i]*eigenVectors[0][0][i];
		y_array_1[i] = eigenVectors[2][0][i]*eigenVectors[2][0][i];
		x_array_2[i] = i;
	}
	for(int i = 0; i < 10; i++){
		cout << "\n" << i << "\n";
		cout << energies[0][i]<<endl;
		cout << energies[1][i]<<endl;
		cout << energies[2][i]<<endl;
	}
	gnuplot_two_functions ("Numerical Eigenfunctions", "linespoints", "x", "Eigenfunction |E_1> (x)",
		x_array_2, y_array_2, number_steps, "Ground", x_array_2, y_array_1, number_steps, "HH ground");
	
	electric_field = 0.00065;
	//ShiftBandgapQWI(QW, (double)params.targetBandgapShift);
	plot_potential(QW);
	solve(QW, x, energies, eigenVectors);
	...
	for (int i = 0; i < number_steps; i++) {
		y_array_2[i] = eigenVectors[0][0][i]*eigenVectors[0][0][i];
		y_array_1[i] = eigenVectors[2][1][i]*eigenVectors[2][1][i];
		x_array_2[i] = i;
	}

	gnuplot_two_functions ("Numerical Eigenfunctions", "linespoints", "x", "Eigenfunction |E_1> (x)",
		x_array_2, y_array_2, number_steps, "Ground", x_array_2, y_array_1, number_steps, "HH Excited 1");	
	//~ for (int i = 0; i < number_steps; i++) {
		//~ y_array_2[i] = eigenVectors[1][0][i]*eigenVectors[1][0][i];
		//~ y_array_1[i] = eigenVectors[2][0][i]*eigenVectors[2][0][i];
		//~ x_array_2[i] = i;
	//~ }
	//~ gnuplot_two_functions ("Numerical Eigenfunctions", "linespoints", "x", "Eigenfunction |E_1> (x)",
	...
	//~ for (int i = 0; i < number_steps; i++) {
		//~ y_array_2[i] = eigenVectors[0][0][i]*eigenVectors[0][0][i];
		//~ y_array_1[i] = eigenVectors[0][1][i]*eigenVectors[0][1][i];
		//~ x_array_2[i] = i;
	//~ }
	//~ gnuplot_two_functions ("Numerical Eigenfunctions", "linespoints", "x", "Eigenfunction |E_1> (x)",
		//~ x_array_2, y_array_2, number_steps, "Ground", x_array_2, y_array_1, number_steps, "5th excited state");
	...
	//~ electric_field = 0.00;
	//~ for (int i = 0; i < number_steps; i++) {
		//~ cout << QW.eff_mass(0, x[i]) << " " << QW.eff_mass(1, x[i]) << " < ! > " << QW.eff_mass(2, x[i]) << endl;
	//~ }	

	//~ cout << "final bandgap: " << h_c / abs(energies_relative[0][0]-energies_relative[2][0])<<endl;
	...
	//~ cout << "final bandgap: " << h_c / E_GAP[0][0][1] <<endl;
	//~ vector<vector<vector<double>>> I_squared_matrix = findOverlapsOnly(eigenVectors);
	//~ cout << "final bandgap: " << I_squared_matrix[0][0][0] <<endl;
}
